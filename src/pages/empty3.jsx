// src/pages/Dashboard.jsx here water intake delte before code 
  ...
  <div className="space-y-2">
    {meals.map((meal, index) => (
      <div key={index} className="flex justify-between items-center">
        ...
        <span className={`text-sm px-2 py-1 rounded ${meal.taken ? "bg-green-100 text-green-800" : "bg-gray-100 text-gray-800"}`}>
...
export default function Dashboard() {
  ...
  // Normalize all tracker structures
  const normalize = (raw) => {
    if (!raw) return [];
  ...
  // Merge trackers from both contexts
  const mergeTrackers = () => {
  ...
  const convertToLitres = (entry) => {
    ...
    return (Number(raw) || 0) / 1000; // default ml → L
  };

  const waterSeries = (data.trackers.water || []).slice(-7).map((t) => ({
    date: new Date(t.date).toLocaleDateString(),
    value: convertToLitres(t)
  }));

  const latestWater = latest("water");
  const currentWaterLitres = convertToLitres(latestWater);
  const waterPercentage = Math.round((currentWaterLitres / 8) * 100) || 0;
  ...
  // EXERCISE FIX (steps removed)
  const latestExercise = latest("exercise");
  const exerciseMinutes =
    typeof latestExercise?.value === "object"
      ? Number(latestExercise.value.duration || latestExercise.value.minutes || 0)
      : Number(latestExercise?.value || 0);

  const exercisePercent = Math.round((exerciseMinutes / 60) * 100) || 0;
  ...
  const stats = {
    medicines: {
      ...
      taken: data.reminders.filter((r) => r.status === REMINDER_STATUS.TAKEN).length
    },
    ...
    vitals: latest("vitals")?.value || { bp: "N/A", hr: "N/A", temp: "N/A" },
    meals: latest("meals")?.value || [
    ...
    <div id="dashboard-root" className="space-y-6">
      ...
      <div className="dashboard-card bg-white dark:bg-gray-800 rounded-xl p-6">
        <div className="flex justify-between items-center">
          ...
          <Link to={ROUTES.BADGES} className="text-blue-500 text-sm">
            View All
          </Link>
        </div>
        <div className="flex flex-wrap gap-2 mt-4">
          {data.badges?.slice(0, 3).map((b) => (
            <div key={b.id} className="px-3 py-1 bg-blue-100 dark:bg-blue-900 text-blue-800 dark:text-blue-100 rounded-full text-sm">
              {b.name}
            </div>
          ))}
          {!data.badges?.length && <p className="text-sm text-gray-500">No badges earned yet</p>}
        </div>
      </div>
        ...
        <div className="p-4 bg-white dark:bg-gray-800 rounded shadow">
          ...
          {rquote ? (
            <blockquote className="italic text-gray-700 dark:text-gray-200">
              "{rquote.text}" —
              <span className="font-semibold not-italic">{rquote.author}</span>
            </blockquote>
          ) : (
