...
const MedicineScheduleItem = ({ name, time, status }) => (
  <div className="schedule-item flex justify-between items-center p-4 border-b border-gray-100 dark:border-gray-700 last:border-b-0">
    ...
    <span className={`status-pill ${status === 'taken' ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800'} px-3 py-1 rounded-full text-xs font-medium`}>
...
export default function Dashboard() {
  const { state: _state } = useData(); // Prefix with underscore
...
const rquote = pickRandom(quote); // Note: change healthQuotes to quote since that's how it's imported
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        setLoading(true);
        const [medicines, trackers, reminders, appointments, badges] = await Promise.all([
          localforage.getItem('medicines'),
          localforage.getItem('trackers'),
          localforage.getItem('reminders'),
          localforage.getItem('appointments'),
          localforage.getItem('badges')
        ]);
        ...
        setData({
          medicines: medicines || [],
          trackers: {
            water: groupedTrackers.water || [],
            sleep: groupedTrackers.sleep || [],
            exercise: groupedTrackers.exercise || [],
            mood: groupedTrackers.mood || [],
            meals: groupedTrackers.meals || [],
            vitals: groupedTrackers.vitals || []
          },
          reminders: reminders || [],
  ...
  const getLatestTrackerData = (type) => {
    const typeData = data.trackers[type] || [];
    return typeData.sort((a, b) => new Date(b.date) - new Date(a.date))[0];
  };
  ...
  const stats = {
    ...
    vitals: getLatestTrackerData('vitals')?.value || {
      bp: 'N/A',
      hr: 'N/A',
      temp: 'N/A'
    },
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="dashboard-card bg-white dark:bg-gray-800 rounded-xl p-6">
          ...
          <div className="space-y-4">
            <MedicineScheduleItem name="Vitamin D" time="08:00" status="taken" />
            <MedicineScheduleItem name="Blood Pressure" time="12:00" status="pending" />
            <MedicineScheduleItem name="Omega-3" time="18:00" status="pending" />
          </div>
        </div>
